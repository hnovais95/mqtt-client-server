...
namespace Server.Infra.Db
{
    ...
    public class RainRepository : IRepository<RainSensor>
    {
        public async Task<IEnumerable<RainSensor>> GetAll()
        {
            using var conn = DbFactory.CreateConnection();
            return await conn.GetAllAsync<RainSensor>();
        }

        public void Insert(RainSensor sensor, int timeout = 0)
        {
            using var conn = DbFactory.CreateConnection();
            conn.Open();
            ...
            cmd.Parameters.Add(DbFactory.CreateParameter("rain", sensor.Rain, DbType.Boolean));
